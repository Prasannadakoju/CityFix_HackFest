...
$sql = "SELECT * FROM complaints WHERE status='Default'";
...
if ($result->num_rows > 0):
    $latestProblems = array();
    while ($row = $result->fetch_assoc()):
        // Store each row in an array
        $latestProblems[] = $row;
    endwhile;

    // Sort the array based on the 'created_at' field in descending order
    usort($latestProblems, function($a, $b) {
        return strtotime($b['created_at']) - strtotime($a['created_at']);
    });

    // Display the sorted problems
    foreach ($latestProblems as $row):
?>
        <div class="card" style="background-color: #c6e9e9;margin-bottom:20px;">
            ...
            <div class="card-body">
                ...
                <!-- Arrow link -->
                <a href="singleproblem.php?id=<?php echo $row["id"]; ?>" class="arrow-link">
